...
interface CategoryBuyPageProps {
  params: {
...
async function CategoryBuyList({ params, searchParams }: CategoryBuyPageProps) {
  ...
  query = query.where("Ticket.ticket_type", "=", "buy");
  countQuery = countQuery.where("Ticket.ticket_type", "=", "buy");
...
async function CategoryBuyListSkeleton() {
  return (
    <section className="grid grid-cols-12 gap-4">
      {[...Array(20)].map((_, i) => (
